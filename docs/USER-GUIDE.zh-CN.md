...
> **长对话怎么办？** 超长会话的接力包会自动精简为"提要 + 首尾关键轮次"，
> 但同时导出**未删节全文**（同名 `-full.md`），并在包头注明路径——
> 目标 AI 能读本地文件的话（Claude Code / Codex 等都能），它会自己去读全文，
> 包内摘录只当目录用。同属某个项目的对话还会注明**所属工作空间**，
> 提示目标 AI 顺带读取该目录获得代码层上下文。
